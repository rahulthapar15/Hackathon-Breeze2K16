...
/**
 * Some modifications by : Abhinav Puri (pabhinav@iitrpr.ac.in)
 *
 * @author romannurik (http://roman.nurik.net/)
 */
public class AnimatedLogoView extends View {

    private static final int SPEED_INVERSE_FACTOR = 1;
    private static final int TRACE_TIME = 2000 * SPEED_INVERSE_FACTOR;
    private static final int TRACE_TIME_PER_GLYPH = 1000 * SPEED_INVERSE_FACTOR;
    private static final int FILL_START = 1200 * SPEED_INVERSE_FACTOR;
    private static final int FILL_TIME = 2000 * SPEED_INVERSE_FACTOR;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mGlyphData.length; i++) {
            float phase = constrain(0, 1, (t - (TRACE_TIME - TRACE_TIME_PER_GLYPH) * i * 1f / mGlyphData.length) * 1f / TRACE_TIME_PER_GLYPH);
